...
db_connection = mysql.connector.connect(
    ...
    password="guru",  # MySQL password
    database="HOTEL_MANAGEMENT"  # The database name you created earlier
)

# Create a cursor object to interact with the database
db_cursor = db_connection.cursor()

db_cursor.execute("CREATE DATABASE IF NOT EXISTS HOTEL_MANAGEMENT")
db_cursor.execute("USE HOTEL_MANAGEMENT")
db_cursor.execute('''CREATE TABLE IF NOT EXISTS RECORDS(NAME char(50), PHONE_NUMBER char(10), EMAIL_ID char(50),
                    PASSWORD char(50))''')
...
def sign_up() :
     ...
     na = 0
     db_cursor.execute("SELECT PHONE_NUMBER FROM RECORDS")
     st = db_cursor.fetchall()
     ...
     if na != 2 :
            ...
            db_cursor.execute("INSERT INTO RECORDS VALUES('" + n1 + "','" + (str)(ph1) + "','" + em1 + "','" + pswd1 + "')")
...
def log_in() :
      ...
      if na != 1 :                 
             ...
             while c < 3 :
                   ...
                   db_cursor.execute("SELECT PASSWORD FROM RECORDS WHERE PHONE_NUMBER = '" + (str)(ph1) + "'")
                   st = db_cursor.fetchall()
                   if st[0][0] == pswd2 :
                         c = 4
                   c = c + 1
...
def card_valid(cval):
    ...
    if len(cval) < 13 or len(cval) > 19:
        print("Invalid input: Card number must be between 13 and 19 digits.")
        return False

    print("Card number format is valid.")
    return True
...
while True :
       ...
       def rooms() :
              ...
              db_cursor.execute("SELECT NAME, EMAIL_ID, PASSWORD FROM RECORDS WHERE PHONE_NUMBER = '" + (str)(ph1) + "'")
              for i in db_cursor :
                     nm = i[0]
                     emid = i[1]
                     pswd = i[2]
       ...
       def account_details() :
             ...
             if cnf == 1 :
                   newnm = input("\nEnter the new NAME : ")
                   db_cursor.execute("UPDATE RECORDS SET NAME = '" + newnm + "' WHERE PHONE_NUMBER = '" + (str)(ph1) + "'")
                   ...
                   db_cursor.execute("UPDATE RECORDS SET EMAIL_ID = '" + newemid + "' WHERE PHONE_NUMBER = '" + (str)(ph1) + "'")
                   db_cursor.execute("COMMIT")
                  ...
                  db_cursor.execute("UPDATE RECORDS SET PASSWORD = '" + newpswd + "' WHERE PHONE_NUMBER = '" + (str)(ph1) + "'")
                  db_cursor.execute("COMMIT")
       ...
       def delete_account() :
             ...
             if cnf == 1 :
                   db_cursor.execute("DELETE FROM RECORDS WHERE PHONE_NUMBER = '" + (str)(ph1) + "'")
                   db_cursor.execute("COMMIT")
...
def add_customer(name, phone, email, address):
    query = """INSERT INTO CUSTOMERS (NAME, PHONE_NUMBER, EMAIL_ID, ADDRESS)
               VALUES (%s, %s, %s, %s)"""
...
def add_room(room_number, room_type, price, status, description):
    query = """INSERT INTO ROOMS (ROOM_NUMBER, ROOM_TYPE, PRICE, STATUS, DESCRIPTION)
               VALUES (%s, %s, %s, %s, %s)"""
...
def make_booking(customer_id, room_id, check_in_date, check_out_date, num_nights, total_price, payment_method, status):
    query = """INSERT INTO BOOKINGS (CUSTOMER_ID, ROOM_ID, CHECK_IN_DATE, CHECK_OUT_DATE, 
                NUMBER_OF_NIGHTS, TOTAL_PRICE, PAYMENT_METHOD, STATUS)
               VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"""
    values = (customer_id, room_id, check_in_date, check_out_date, num_nights, total_price, payment_method, status)
    
    try:
        db_cursor.execute(query, values)
        db_connection.commit()
        print(f"✅ Booking made successfully for Customer ID {customer_id}.")
    except Exception as e:
        print(f"❌ Booking failed due to error: {e}")



def make_payment(booking_id, payment_date, payment_amount, payment_method, payment_status):
    query = """INSERT INTO PAYMENTS (BOOKING_ID, PAYMENT_DATE, PAYMENT_AMOUNT, PAYMENT_METHOD, PAYMENT_STATUS)
               VALUES (%s, %s, %s, %s, %s)"""
    values = (booking_id, payment_date, payment_amount, payment_method, payment_status)
    db_cursor.execute(query, values)
    db_connection.commit()
    print(f"Payment for Booking ID {booking_id} recorded successfully.")
...
make_booking(1, 101, "2025-07-18", "2025-07-20", 2, 8000.0, "Credit Card", "Confirmed")
